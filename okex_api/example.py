...
def chengjiao():
    ...
    while(True):
        ...
        time1.sleep(1000)
        result = swapAPI.get_trades('BTC-USDT-SWAP')
        data = json.loads(json.dumps(result))
        for chengjiao in data:
            size = chengjiao['size']
            price = chengjiao['price']
            side = chengjiao['side']
            timestamp = chengjiao['timestamp']
            if side == 'sell':
                stadu = '----卖出/'
                sellsum += int(size)
            else:
                stadu = '----买入/'
                buysum += int(size)
            shuchu = timestamp + '----'+ price + stadu + size
            print(shuchu)
        #except:
           # print('失败')
        print('买入 ：'+str(buysum)+ '----'+'卖出 ：'+str(sellsum))
...
if __name__ == '__main__':
    ...
    api_key = "eb3dfa06-a4c0-4529-9440-b8ef613ece7d"
    seceret_key = "0CF492F0C721E5D321AB2CF80EB3C280"
    passphrase = "q847749636"
...
# 交割合约API
    ...
    # result = futureAPI.get_fills('BTC-USDT-SWAP')
...
# 永续合约API
    ...
    # result = swapAPI.get_fills('BTC-USDT-SWAP')
    # 获取合约挂单冻结数量 （5次/2s）
